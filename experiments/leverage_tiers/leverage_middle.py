"""Leverage comparison: Conservative vs Middle vs Aggressive on 5-asset weekly."""
...
def main():
    ...
    pr("=" * 80)
    pr("  LEVERAGE COMPARISON: Conservative vs Middle vs Aggressive")
    pr("=" * 80)
    ...
    pr("-" * 80)
    pr(f"  {'Metric':<18} {'1x':>10} {'Conserv':>10} {'Middle':>10} {'Aggress':>10}")
    pr(f"  {'-'*18} {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
    ...
    for cname, cs, ce in [("GFC","2008-09-01","2009-03-31"),("COVID","2020-02-19","2020-03-23"),("2022 Bear","2022-01-03","2022-10-31")]:
        pr(f"\n  {cname}:")
        pr(f"  {'Metric':<12} {'1x':>10} {'Conserv':>10} {'Middle':>10} {'Aggress':>10}")
        pr(f"  {'-'*12} {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
    ...
    pr(f"{'='*80}")
    pr(f"  {'Year':>6} {'1x':>9} {'Conserv':>9} {'Middle':>9} {'Aggress':>9} {'IVV':>9}")
    ...
    pr(f"  RECOMMENDATION")
    pr(f"{'='*80}")
    pc = perfs["Conservative"]; pm = perfs["Middle"]; pa = perfs["Aggressive"]
    pr(f"\n  {'Config':<14} {'Sharpe':>8} {'Return':>8} {'MaxDD':>8} {'Terminal':>10}")
    pr(f"  {'-'*14} {'-'*8} {'-'*8} {'-'*8} {'-'*10}")
    for name in ["Conservative","Middle","Aggressive"]:
        p = perfs[name]
    ...
    if sh_m > sh_c:
        pr(f"\n  → Middle (1.25x/1.75x) has BETTER Sharpe than Conservative ({sh_m:.3f} vs {sh_c:.3f})")
        ...
        pr(f"\n  → Middle and Conservative have SIMILAR Sharpe ({sh_m:.3f} vs {sh_c:.3f})")
        pr(f"    Middle offers ${pm['final']-pc['final']:+.2f} more terminal wealth at {pm['dd']-pc['dd']:+.1%} more drawdown.")
    else:
        pr(f"\n  → Conservative still has BETTER Sharpe ({sh_c:.3f} vs {sh_m:.3f})")
        pr(f"    The Sharpe curve is monotonically decreasing with leverage.")

    rp = OUTPUT / "leverage_middle_report.txt"
    with open(rp, "w") as f:
        f.write("\n".join(lines))
    print(f"\n  Report saved: {rp}")
